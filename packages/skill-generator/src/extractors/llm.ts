 ...
 */
function validateLearning(raw: unknown): Learning | null {
  ...
  if (!validCategories.includes(obj.category as LearningCategory)) return null;
  ...
  return {
    category: obj.category as LearningCategory,
...
export async function summarizeSessions(
  ...
  // Group by category
  const byCategory: Record<LearningCategory, Learning[]> = {
    decision: [],
    pattern: [],
    gotcha: [],
    convention: [],
