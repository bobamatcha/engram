 ...
 */
export function extractPatterns(sessions) {
    const fileCoEdits = new Map();
    const toolSequences = [];
    const testCommands = [];
    const buildCommands = [];
    const errorPatterns = [];
    // Track tool sequences per session
    const toolSeqCounts = new Map();
    for (const session of sessions) {
        const filesEditedInSession = new Set();
        const toolsInSession = [];
 ...
 */
export function matchesWorkspace(session, targetPath) {
