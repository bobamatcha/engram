 ...
 */
export function detectErrorResolution(session) {
    let errorCount = 0;
    let fixCount = 0;
    for (let i = 0; i < session.messages.length; i++) {
        ...
        if (msg.content) {
            const hasError = /error[:\s]/i.test(msg.content) ||
 ...
 */
export function detectNonObviousDiscovery(session) {
    ...
    const readCount = session.messages.reduce((count, msg) => {
        return (count + (msg.toolCalls?.filter((t) => ['Read', 'read', 'Grep', 'grep'].includes(t.name)).length ?? 0));
    }, 0);
    // Look for investigation patterns
    const hasSearchPattern = session.messages.some((msg) => msg.toolCalls?.some((t) => ['Grep', 'grep', 'Glob', 'glob', 'search'].includes(t.name.toLowerCase())));
 ...
 */
export function detectArchitecturalDecision(session) {
    ...
    for (const msg of session.messages) {
        if (msg.role === 'user' && msg.content) {
            ...
            const hasDecisionVerb = /should (we|i)/i.test(msg.content) ||
 ...
 */
export function detectConventionEstablishment(session) {
 ...
 */
export function detectWorkaroundDiscovery(session) {
 ...
 */
export function detectAllSignals(session) {
    return [
