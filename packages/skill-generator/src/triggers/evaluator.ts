...
export function evaluateTrigger(
  ...
  const signalsByType = new Map<string, TriggerSignal[]>();
  ...
  const aggregatedSignals: TriggerSignal[] = [];
  for (const [type, signals] of signalsByType) {
    ...
    aggregatedSignals.push({
      type: type as TriggerSignal['type'],
      triggered,
 ...
 */
export async function evaluateTriggerWithLLM(
  sessions: UnifiedSession[],
  llmClient: LlmClient,
  options: { threshold?: number } = {}
 ...
 */
export function hasObviousSkillContent(sessions: UnifiedSession[]): boolean {
  for (const session of sessions) {
    ...
    const hasFix = session.messages.some((m) =>
      m.toolCalls?.some((t) => ['Edit', 'Write', 'edit', 'write'].includes(t.name))
    );

    if (hasError && hasFix) return true;

    // Check for architectural discussion
    const hasArchitectural = session.messages.some(
      (m) =>
        m.content &&
        m.role === 'user' &&
    ...
    if (hasArchitectural) return true;
  }
