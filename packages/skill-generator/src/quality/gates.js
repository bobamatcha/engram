 ...
 */
export function checkReusable(learning) {
    // Learnings with high confidence and clear patterns are more reusable
    const hasGeneralTerms = /always|never|should|must|prefer|avoid|when|if/i.test(learning.summary) ||
        /pattern|convention|approach|method|technique/i.test(learning.summary);
    const isSpecificToInstance = /this specific|this particular|in this case only/i.test(learning.summary) ||
 ...
 */
export function checkNonTrivial(learning) {
 ...
 */
export function checkSpecific(learning) {
    const hasActionableVerb = /use|add|remove|change|update|configure|set|enable|disable|call|invoke|wrap/i.test(learning.summary);
    const hasConcreteDetail = learning.detail !== undefined ||
        (learning.files && learning.files.length > 0) ||
        /`[^`]+`/.test(learning.summary); // Has code reference
    const isTooVague = /sometimes|maybe|might|could|generally|usually/i.test(learning.summary) &&
        !learning.detail;
 ...
 */
export function checkVerified(learning) {
 ...
 */
export function checkQuality(learning) {
    return [
 ...
 */
export function passesAllGates(learning) {
