...
import type { Learning, ExtractedPatterns, QualityReport, QualityCheck } from '../types.js';
import { checkQuality, validatePatterns } from './gates.js';

/**
 * Calculate a quality score for a set of learnings
 */
export function scoreLearnings(learnings: Learning[]): number {
 ...
 */
export function scorePatterns(patterns: ExtractedPatterns): number {
  let score = 0;
  let factors = 0;
  ...
  const seqScore =
    patterns.toolSequences.length > 0
  ...
  // Commands (0-0.25)
  const cmdScore =
    Math.min((patterns.testCommands.length + patterns.buildCommands.length) / 5, 1) * 0.25;
  score += cmdScore;
  factors++;
...
export function generateQualityReport(
  ...
  if (learnings.length > 0) {
    ...
    checks.push({
      ...
      reason:
        qualityLearnings.length < learnings.length * 0.5
